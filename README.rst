...
``convert.rb`` reads a My Clippings.txt file from a Kindle, and imports the clippings into a running 'DevonThink'_ database
...
.. _DevonThink: http://www.devon-technologies.com/products/devonthink/

Installation
------------

convert.rb requires `rb-appscript`_
  
   sudo gem install rb-appscript
.. _rb_appscript: http://appscript.sourceforge.net/rb-appscript/install.html
...
You can also use the script on a hierarchy of notes exported from ``Scrivener``_, or any other program that creates a directory
...
I use ``Alloy``_, built on ``YUI``_ to generate the interactive menus, however the menu is laid-out as a simple unordered
